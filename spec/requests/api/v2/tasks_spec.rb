...
RSpec.describe 'Task API' do
  ...
  let(:headers) do
    {
      'Accept' => 'application/vnd.task-manager.v2',
  ...
  describe 'GET /tasks' do
    context 'when no filter params are  sent' do
      before do
    ...
    context 'when filter params are sent' do
      ...
      it 'returns only the matching tasks' do
        returned_tasks_titles = json_body[:data].map { |t| t[:attributes][:title] }
  ...
  describe 'GET /tasks/:id' do
    ...
    it 'returns json for the task' do
